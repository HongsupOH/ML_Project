...
class ImplicitRegression(VectorBasedFunction):
    ...
    def evaluate_fitness_vector(self, individual):
        self.eval_count += 1
        data = self.training_data.x
        m = data.shape[0]
        y_hat =  self.evaluate_fitness_vector_helper(individual,data)
        Cd = y_hat
        Cs = np.zeros(m)
        for _ in range(10):
            ...
            ind = ind.tolist()
            data_s = data[ind]
            ys_hat = self.evaluate_fitness_vector_helper(individual,data_s,ind=ind)
            Cs += ys_hat

        Cs_avg = np.mean(Cs)        
        VarD = (1/10)*(Cs - Cs_avg)**2
        fitness =  self.wb*Cd + self.wv*VarD
        return fitness

    def evaluate_fitness_vector_helper(self, individual,X,ind=None):
        _, df_dx = individual.evaluate_equation_with_x_gradient_at(
            x=X)
        if ind!=None:
           dx_dt = self.training_data.dx_dt[ind]
        else:
           dx_dt = self.training_data.dx_dt
